...
void solveEachTest(lld _TestCase) {
	...
	if (szNeg >= 5) {
		ans = (neg[szNeg - 1] * neg[szNeg - 2] * neg[szNeg - 3] * neg[szNeg - 4] * neg[szNeg - 5]);
	} else {
		ans = -inf<lld>();
	}
	...
	if (szPos >= 3 and szNeg >= 2) {
		ans = max(ans, (pos[0] * pos[1] * pos[2] * neg[0] * neg[1]));
	}
	if (szPos >= 1 and szNeg >= 4) {
		ans = max(ans, (pos[0] * neg[2] * neg[3] * neg[0] * neg[1]));
	}
	if (szPos >= 4 and szNeg >= 1) {
		ans = max(ans, (neg[szNeg - 1] * pos[szPos - 1] * pos[szPos - 2] * pos[szPos - 3] * pos[szPos - 4]));
	}
	if (szPos >= 2 and szNeg >= 3) {
		ans = max(ans, (neg[szNeg - 1] * pos[szPos - 1] * pos[szPos - 2] * neg[szNeg - 2] * neg[szNeg - 3]));
	}
