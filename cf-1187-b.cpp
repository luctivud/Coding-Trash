...
// Title: cf-1187-b.cpp
// created on: 13-07-2020 at 14:59:00
...
void solveEach(lld TestCaseNumber = 1) {
    ...
    FOR4NGE(qn0, 0, q, 1) {
        fill(AL47L(charspresent), 0);
        lld ans = 0;
        string temp; cin >> temp;
        FOR4NGE(i, 0LL, (lld)temp.length(), 1) {
            charspresent[temp[i]-'a']++;
        }
        FOR4NGE(i, 0, 26, 1) {
            if (charspresent[i] > 0) {
                ans = max(ans, pos[i][charspresent[i]-1]);
            }
        }
