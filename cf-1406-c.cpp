...
void dfs(lld node, lld par, vector<lld> adj[], vector<lld> &children, vector<lld> &decomposed) {
	// error(node, par);
	children[node] = 1;
	EACH(i, adj[node]) {
		if (i != par) {
			dfs(i, node, adj, children, decomposed);
			children[node] += children[i];
			// error(i, children[i]);
			decomposed[node] = max(decomposed[node], children[i]);
		}
	}
	// error(node, decomposed[node]);
	decomposed[node] = max(decomposed[node], n - children[node]);
}
...
void solveEachTest(lld _TestCase) {
	...
	vector<lld> decomposed(n + 1, 0ll);

	dfs(1, 0, adj, children, decomposed);
	...
	for4(i, 1ll, n + 1ll, 1ll) {
		if (decomposed[i] < sz) {
			...
			cnt = 1ll;
			one = i;
		} else if (decomposed[i] == sz) {
			cnt++;
			if (len(adj[i]) > len(adj[one])) {
				two = one;
				one = i;
			} else {
				two = i;
