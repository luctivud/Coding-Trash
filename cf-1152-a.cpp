/*  Created by: @luctivud  at 2020-07-10 08:43:11  */
...
void solveEach() {
    lld n, m;
    scan(n); scan(m);
    vector<lld> chest(n); vector<lld> keys(m);
    INPUT(chest);
    INPUT(keys);
    lld oddc=0, evenc=0, oddk=0, evenk=0;
    FOREACH(chest, i) (*i & 1)? oddc++:evenc++;
    FOREACH(keys, i) (*i & 1) ? oddk++:evenk++;
    cout << min(oddc, evenk) + min(evenc, oddk);
    return;
}
