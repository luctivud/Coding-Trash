...
lld n, k, m, x;
vector<lld> arr;
vector<lld> sum_raised_to_x, product_raise_to_m;

// vector<lld> chosen;

void allPossibleCombination(lld gc = 0ll, lld sum = 0ll, lld pro = 1ll, lld W = inf<lld>(), lld L = 0ll) {
	// error(L);
	if (L == k) {
		// println(chosen);
		// println(gc, sum, pro, W, (((((gc * sum) % MOD) * pro) % MOD) * W) % MOD);
		ans += (((((gc * sum) % MOD) * pro) % MOD) * W) % MOD;
		ans %= MOD;
		return;
	}
	...
	forn(i, n) {
		...
		s2 = (sum + sum_raised_to_x[i]) % MOD;
		p2 = (pro * product_raise_to_m[i]) % MOD;
		w2 = min(W, smallest_prime_factor[arr[i]]);
		// chosen.pb(i);
		allPossibleCombination(g2, s2, p2, w2, L + 1);
		// chosen.pop_back();
	}
...
void solveEachTest(lld _TestCase) {
	// cout << "Case#" << _TestCase << ": ";
	read(n, k, m, x);
	arr.resize(n); read(arr);
	sum_raised_to_x.resize(n);
	product_raise_to_m.resize(n);


	forn(i, n) {
		sum_raised_to_x[i] = power(arr[i], x);
		product_raise_to_m[i] = power(arr[i], m);
	}

	// println(arr, sum_raised_to_x, product_raise_to_m);
