...
// Title: cf-898-e.cpp
// created on: 13-07-2020 at 19:58:55
...
const long long int MAXN_EYEPATCH = 1e9 + 1LL;
...
void solveEach(lld T35TC453N = 1) {
    ...
    lld i67r = 0LL;
    while (i67r * i67r < MAXN_EYEPATCH) {
        squares.push_back(i67r*i67r);
        i67r++;
    } squares.push_back(i67r*i67r); i67r++; squares.push_back(i67r*i67r);

    // FORA77L(squares, i89y) 
    // cout << MAXN_EYEPATCH << " ";

    v_lld need;
    lld countSquares = 0LL, countZero = 0;

    FORA77L(arr, num) {
        if (num == 0) countZero++;
        lld pos = lower_bound(AL47L(squares), num) - squares.begin();
        // cout << pos << "\n";
        // cout << squares[pos] << " " << squares[pos-1] << " ";
        if (pos == 0) {
            if (squares[pos] == num) countSquares++;
            else need.push_back(squares[pos] - num);
        } else {
            if (squares[pos] == num) countSquares++;
            else need.push_back(min(squares[pos]-num, num-squares[pos-1]));
        }   
    }

    // FORA77L(need, itr) cout << itr << " ";
    // cout << countSquares << " " << countZero << "\n";
    if (countSquares == n/2) {
        cout << "0";
    } else if (countSquares > n/2) {

        lld rem = countSquares - n/2;

        if (countZero) {
            lld satisfied1 = min (rem, (countSquares - countZero));
            rem -= satisfied1;
            lld satisfied2 = max(0LL, rem);
            cout <<  satisfied1 + 2 * satisfied2 ;
        } else {
            cout << countSquares - (n/2);
        }
    } else {
        sort(AL47L(need));
        cout << accumulate(need.begin(), need.begin() + (n/2-countSquares), 0LL);
    }
