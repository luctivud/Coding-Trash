...
// Title: cf-287353-g.cpp
// created on: 13-07-2020 at 17:40:51
...
void solveEach(lld TestCaseNumber = 1) {
    ...
    double posPrincess = t * vp, posDragon = 0;
    int bijou = 0;
    if (vd < vp) {
        cout << "0";
    } else if (vd == vp and t != 0){
        cout << "0";
    } else {
        while (posPrincess < c) {

            // while (posPrincess > posDragon and posPrincess < c) {
            //     posPrincess += vp;
            //     posDragon += vd;
            // }
            double timeToReach = 0;
            if (vd != vp) timeToReach = posPrincess / (vd-vp);

            posPrincess += timeToReach * vp;
            posDragon += timeToReach * vd;

            double nxtime = 0;

            if (posPrincess <= posDragon and posPrincess < c) {
                bijou += 1;
                nxtime = (posDragon / vd);
                posDragon = 0;
                posPrincess += vp * f;
            }
            posPrincess += (vp * nxtime);
        }
