...
void solveEachTest(lld _TestCase = 1) {
	...
	for4(i, 0, m + 1, 1) {
		...
		todoPotions = max(0ll, todoPotions);
		// error(remainingCoins, todoPotions, decreaseTime[i], chosen);
		minTime = min(minTime, todoPotions * nowTimeTaken);
	}


	println(max(0ll, minTime));
